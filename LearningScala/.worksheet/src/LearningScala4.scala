object LearningScala4 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(285); 
 ...
 for (ship <- shipList) {println(ship)};$skip(188); 
...
val exerciseList = (1 to 20).toList;System.out.println("""exerciseList  : List[Int] = """ + $show(exerciseList ));$skip(49); 
exerciseList.filter(_ % 3 == 0).foreach(println)}
