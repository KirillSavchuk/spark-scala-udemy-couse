object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  transformInt(2, x => {val y = x * 2; y * y});System.out.println("""res2: Int = """ + $show(res$2));$skip(407); 
  ...
  def upper(s: String) : String = {
  	s.toUpperCase
  };System.out.println("""upper: (s: String)String""");$skip(30); 
  
  println(upper("hello!"));$skip(82); 
  
  def transfromString(s: String, f: String => String) : String = {
  	f(s)
  };System.out.println("""transfromString: (s: String, f: String => String)String""");$skip(60); 
  
  println(transfromString("hello!", s => s.toUpperCase))}
}
